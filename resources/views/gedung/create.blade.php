...
<div class="container">
    ...
    <!-- Formulir untuk menambah gedung -->
    <form action="{{ route('gedung.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="form-group">
            <label for="nama_gedung">Nama Gedung</label>
            <input type="text" name="nama_gedung" id="nama_gedung" class="form-control @error('nama_gedung') is-invalid @enderror" value="{{ old('nama_gedung') }}" required>
            @error('nama_gedung')
        ...
        <div class="form-group">
            ...
            <input type="number" name="harga_internal" id="harga_internal" class="form-control @error('harga_internal') is-invalid @enderror" value="{{ old('harga_internal') }}" required>
        ...
        <div class="form-group">
            ...
            <input type="number" name="harga_eksternal" id="harga_eksternal" class="form-control @error('harga_eksternal') is-invalid @enderror" value="{{ old('harga_eksternal') }}" required>
        ...
        <div class="form-group">
            ...
            <select name="alamat_id" id="alamat_id" class="form-control @error('alamat_id') is-invalid @enderror" required>
                ...
                @foreach ($alamat as $item)
                    <option value="{{ $item->id }}" {{ old('alamat_id') == $item->id ? 'selected' : '' }}>{{ $item->nama_jalan }}</option>
                @endforeach
            </select>
            @error('alamat_id')
                <div class="invalid-feedback">{{ $message }}</div>
